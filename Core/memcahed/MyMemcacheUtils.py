...
class Client(threading.local):
    ...
    def get_keys(self, id_list):
        ...
        for s in self.servers:
            ...
            for slab_id in id_list:
                serverData = []
                data.append((name, serverData))
                s.send_cmd('stats cachedump %d 10000' % slab_id)
                readline = s.readline
                while 1:
                    line = readline()
                    if not line or line.strip() == 'END':
...
def _doctest():
    ...
    mc = MyMemcacheUtils.Client(servers, debug=1)
    globs = {"mc": mc}
    results = doctest.testmod(MyMemcacheUtils, globs=globs)
    mc.disconnect_all()
